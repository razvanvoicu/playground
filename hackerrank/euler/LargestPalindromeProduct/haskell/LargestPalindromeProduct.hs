...
main = do
  ...
  forM_ [1..count] $ \_ -> do
    ...
    let result = head $ largestPalindrome n
    putStrLn $ show $ result

largestPalindrome :: Int -> [Int]
largestPalindrome i = dropWhile (not . two3digitFactors) l
  where k = i `div` 1000
        m = [k,k-1..100]
        p = map (\x -> (read $ (show x) ++ reverse (show x) :: Int)) m
        l = dropWhile (> i) p

factorProd k = map (\f -> dropWhile (\x -> x < 100 || x >= 1000 || k `div` x < 100 || k `div` x >= 1000) $ map product $ tails f) $ factorPerm k

two3digitFactors k = (filter (/=[]) $ factorProd k) /= []
