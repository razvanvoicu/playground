...
type SArray t = IOArray Int t
type DArray t = IOArray Int (IOArray Int t)
...
main :: IO ()
main = do 
    [iS,iL,iA,iT] <- map read . words <$> getLine :: IO [Int]
    lct <- newListArray (0,iA-1) =<< replicateM iA (newArray (0,iS-1) NilNode) :: IO (DArray Node)
    deg <- newListArray (0,iA-1) =<< replicateM iA (newArray (0,iS-1) 0) :: IO (DArray Int8)
    putStrLn "Done"
...
readInfos :: Int -> DArray Node -> DArray Int8 -> IO (SArray Info)
readInfos iL lct deg = error ""
